...
def test_effective_priority_aging_bounds() -> None:
    now_ms = 1_000_000
    e = Event(type="task", priority=40, ts_ms=now_ms - 130_000)
    assert effective_priority(e, now_ms) == 36
    e2 = Event(type="task", priority=40, ts_ms=now_ms - 2_000_000)
    assert effective_priority(e2, now_ms) == 5
    e3 = Event(type="task", priority=0, ts_ms=now_ms - 2_000_000)
